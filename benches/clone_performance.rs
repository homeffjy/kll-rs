...
fn main() {
    ...
    println!("Results:");
    println!("========");
    println!("New copy() method:           {:?} ({:.2} μs per operation)", 
             copy_duration, copy_duration.as_micros() as f64 / num_iterations as f64);
    println!("Old serialize/deserialize:   {:?} ({:.2} μs per operation)", 
             serialize_duration, serialize_duration.as_micros() as f64 / num_iterations as f64);
    println!("Current Clone trait:         {:?} ({:.2} μs per operation)", 
             clone_trait_duration, clone_trait_duration.as_micros() as f64 / num_iterations as f64);
    ...
    for &fraction in &test_fractions {
        ...
        println!("  {}% quantile - Original: {:.2}, Copy: {:.2}, Serialize: {:.2}, Clone: {:.2}", 
                 (fraction * 100.0) as u32, original_q, copy_q, serialize_q, clone_q);
        
        assert!((original_q - copy_q).abs() < 1e-10, "Copy quantile mismatch");
        assert!((original_q - serialize_q).abs() < 1e-10, "Serialize quantile mismatch");
        assert!((original_q - clone_q).abs() < 1e-10, "Clone quantile mismatch");
    }
...
fn test_serialization_size_vs_k_parameter() {
    ...
    for &k in &k_values {
        ...
        println!("  K={:>3} -> {:>6} bytes ({:>6.2} KB) | Retained: {:>5} | Estimation mode: {}", 
                 k, size_bytes, size_kb, sketch.get_num_retained(), sketch.is_estimation_mode());
    }
}
...
fn test_memory_efficiency_analysis(sketch: &KllDoubleSketch) {
    ...
    println!("  Values retained:          {:>10} values ({:.2}% retention rate)", num_retained, retention_rate);
    println!("  Raw data size:            {:>10} bytes ({:.2} MB)", raw_data_size, raw_data_size as f64 / 1_048_576.0);
    println!("  Retained data size:       {:>10} bytes ({:.2} KB)", retained_data_size, retained_data_size as f64 / 1024.0);
    println!("  Serialized sketch size:   {:>10} bytes ({:.2} KB)", serialized_size, serialized_size as f64 / 1024.0);
    println!("  Compression ratio:        {:>10.2}x (vs raw data)", compression_ratio);
    println!("  Bytes per retained value: {:>10.2} bytes", bytes_per_retained);
    ...
    println!("  Deserialization time:     {:>10.2} μs", deserialize_time.as_micros() as f64);
    ...
    if serialized_size < retained_data_size as usize {
        ...
        println!("  Storage overhead:         {:>10.2}% savings vs retained raw data", overhead_reduction_pct);
    } else {
        let overhead = serialized_size as f64 - retained_data_size as f64;
        let overhead_pct = overhead / retained_data_size as f64 * 100.0;
        println!("  Storage overhead:         {:>10.2}% extra vs retained raw data", overhead_pct);
    }
}
