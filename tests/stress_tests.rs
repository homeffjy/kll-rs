...
fn test_boundary_conditions_and_edge_cases() {
    ...
    assert!(
        result.is_nan(),
        "Empty sketch should return NaN for quantile"
    );
    ...
    assert!(
        quantiles.is_empty(),
        "Empty sketch should return empty quantiles"
    );

    // Test evenly spaced quantiles on empty sketch
    let evenly_spaced = empty_sketch.get_quantiles_evenly_spaced(5);
    assert!(
        evenly_spaced.is_empty(),
        "Empty sketch should return empty evenly spaced quantiles"
    );
...
fn test_serialization_edge_cases() {
    ...
    for (i, invalid_data) in invalid_data_cases.iter().enumerate() {
        match KllFloatSketch::deserialize(invalid_data) {
            Ok(_) => println!(
                "Unexpectedly succeeded deserializing invalid data case {}",
                i
            ),
...
fn test_clone_and_copy_edge_cases() {
    ...
    assert_eq!(
        large_sketch.get_num_retained(),
        cloned_large.get_num_retained()
    );
...
fn test_panic_safety() {
    ...
    println!(
        "Sketch still functional after panic test, median: {}",
        new_quantile
    );
